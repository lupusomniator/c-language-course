...
+	из любой вершины может выходить максимум 2 узла;
...
<img src="image/1.jpg" widdth="100" height="200">
  ...
  Теперь можно сказать, АВЛ дерево – это двоичное дерево поиска с одним дополнением, оно является сбалансированным:
  ### :exclamation: для любого узла дерева высота его правого поддерева отличается от высоты левого поддерева не более чем на единицу. :exclamation:
...
## 2. Вставка элемента
  ...
  Второй: ищем место куда добавить элемент – спускаемся вниз по дереву, пока не найдем нужное место (справа от узла - если значение переменной больше или равно, слева - если меньше). Нашли – закрепили. 
...
## 4. Большие повороты
  ...
  Реализуются на основе двух маленьких поворотов. Какой именно надо делать зависит от того, какие в данный момент высоты у поддеревьев ниже. Описывать словами все варианты не самая хорошая идея, ниже рассмотрены все случаи:
```c++
...
Основной выигрыш, получаемый от использование АВЛ дерева, является поиск в нём элемента.
