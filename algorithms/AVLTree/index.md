...
Пример: возьмём пустое двоичное дерево. Сложность поиска в таком виде напрямую зависит от высоты пути до нужного элемента - О(h). Если мы рассмотрим монотонно возрастающую последовательность (1, 2, 3, 4 ...) то заметим - все элементы будут добавляться в правую ветку (образуется "сосиска"), высота станет h = n-1 и сложность поиска элемента будет не лучше обыного поиска в массиве. И тут нам на помощь приходит сбалансированное АВЛ дерево, не позволяющее дереву стать "сосиской" т.к. главное правило: разность высот не больше 1 =) 
____
